...
if (isAdmin()) {
    header('Location: ' . getBasePath() . 'admin/profile.php');
    exit;
}
...
// Fetch customer
$stmt = $pdo->prepare('SELECT full_name, email, phone, profile_pic, password FROM users WHERE id = ? AND role = "user"');
$stmt->execute([$userId]);
$customer = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!in_array($mimeType, $allowed)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destPath)) {
                    ...
                    if (!empty($customer['profile_pic'])) {
                        $oldPath = __DIR__ . '/../' . ltrim($customer['profile_pic'], '/');
                        if (file_exists($oldPath)) unlink($oldPath);
                    }
                    $dbPath = 'uploads/profile_pics/' . $filename;
                    $pdo->prepare('UPDATE users SET profile_pic = ? WHERE id = ? AND role = "user"')
    ...
    if ($section === 'remove_avatar') {
        ...
        $pdo->prepare('UPDATE users SET profile_pic = NULL WHERE id = ? AND role = "user"')
            ->execute([$userId]);
    ...
    if ($section === 'password') {
        ...
        if (empty($errors)) {
            if (!password_verify($currentPw, $customer['password'])) {
                $errors[] = 'Current password is incorrect.';
            } else {
                try {
                    $pdo->prepare('UPDATE users SET password = ? WHERE id = ? AND role = "user"')
...
$avatarSrc = !empty($customer['profile_pic']) ? getBasePath() . $customer['profile_pic'] : '';
$hasPhoto  = !empty($customer['profile_pic']);
...
<div class="card profile-wrap">
    ...
    <p style="text-align:center;margin-top:4px;">
        <a href="<?php echo getBasePath(); ?>customer/settings.php"
