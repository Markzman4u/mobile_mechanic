...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($full_name === '' || $problem_type === '') {
        ...
        try {
            ...
            // Create service request — directly in_progress with assigned mechanic
            $stmt = $pdo->prepare(
            ...
            $message = 'Walk-in request created and mechanic assigned. Request ID: ' . $requestId;

            // Reset form values on success
            $full_name = $phone = $email = $address = $problem_type = $description = '';
            $mechanic_id = 0;

            // Refresh mechanic list (one may now be busy)
            $mechanics = $pdo->query('SELECT id, name FROM mechanics WHERE status = "available" ORDER BY name ASC')->fetchAll();
...
<main>
    <div class="card">
        ...
        <form method="post" style="background:#f9f9f9;padding:16px;border-radius:8px;">
            ...
            <label>Phone <input type="tel" name="phone" placeholder="Enter phone number" value="<?php echo e($phone ?? ''); ?>"></label>
