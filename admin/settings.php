...
if (!isAdmin()) {
    header('Location: ' . getBasePath() . 'customer/settings.php');
    exit;
}
...
$stmt = $pdo->prepare('SELECT full_name, email, phone FROM users WHERE id = ? AND role = "admin"');
$stmt->execute([$userId]);
$admin = $stmt->fetch();
        ...
        <div style="max-width:420px;margin:24px 0;">
            <div class="card" style="border-left:4px solid var(--charcoal);">
                ...
                <p style="margin:12px 0 0 0;">
                    <a class="btn btn-primary" href="<?php echo getBasePath(); ?>admin/profile.php">
                        Manage Profile
                    </a>
                </p>
            </div>
