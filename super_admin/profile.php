...
// Fetch super admin
$stmt = $pdo->prepare('SELECT full_name, email, phone, profile_pic, password FROM users WHERE id = ? AND is_superadmin = TRUE');
$stmt->execute([$userId]);
$admin = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($mimeType, $allowed)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $destPath)) {
                    ...
                    $pdo->prepare('UPDATE users SET profile_pic = ? WHERE id = ? AND is_superadmin = TRUE')
    ...
    if ($section === 'remove_avatar') {
        ...
        $pdo->prepare('UPDATE users SET profile_pic = NULL WHERE id = ? AND is_superadmin = TRUE')
            ->execute([$userId]);
    ...
    if ($section === 'profile') {
        ...
        if (empty($errors)) {
            try {
                $pdo->prepare('UPDATE users SET full_name = ?, email = ?, phone = ? WHERE id = ? AND is_superadmin = TRUE')
    ...
    if ($section === 'password') {
        ...
        if (empty($errors)) {
            if (!password_verify($currentPw, $admin['password'])) {
                ...
                try {
                    $pdo->prepare('UPDATE users SET password = ? WHERE id = ? AND is_superadmin = TRUE')
...
<div class="card profile-wrap">
    ...
    <p style="text-align:center;margin-top:4px;">
        <a href="<?php echo getBasePath(); ?>super_admin/settings.php"
